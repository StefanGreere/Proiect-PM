...
DSTATUS disk_initialize(void)
{
	...
	if (ty) {
		// Cardul s-a inițializat cu succes, trecem SPI-ul pe viteză maximă (fosc / 4 = 4 MHz)
		SPCR &= ~(1 << SPR0); // Dezactivăm SPR0 pentru a coborî prescalerul de la 16 la 4
		SPSR |= (1 << SPI2X);  // Opțional: activează dublarea vitezei (fosc / 2 = 8 MHz) pentru performanță maximă
	}
 ...
 */
DRESULT disk_writep(const BYTE *buf, CLUST sa)
{
    DRESULT res;
    WORD bc;
    static WORD wc;
    ...
    {       /* Send data bytes */
        ...
        {   /* Initiate sector write process */
            ...
            if(send_command(CMD24, (DWORD)sa) == 0) 
            {           /* WRITE_SINGLE_BLOCK */
			...
			while(wc > 0) {
                SD_transmit(' '); // Trimitem spatiu ASCII (0x20)
                wc--;
            }
